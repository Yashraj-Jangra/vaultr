...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sessionId || !otp) return NextResponse.json({ error: "sessionId and otp required" }, { status: 400 });
    ...
    if (!data.verificationToken) {
      return NextResponse.json({ error: "No verification code sent. Request a new code." }, { status: 400 });
    }
