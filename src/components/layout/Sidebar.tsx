...
export function Sidebar({ mobileOpen, onClose }: SidebarProps) {
  ...
  const navLinkCls = (active: boolean) =>
    `flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] transition-colors w-full overflow-hidden ${
      active
  ...
  const sectionLinkCls = (active: boolean) =>
    `flex items-center justify-between px-3 py-1.5 rounded-md text-[12px] w-full transition-colors overflow-hidden ${
      active
  ...
  return (
    <>
      ...
      >
        <div className={`flex items-center gap-2.5 h-14 border-b border-[var(--border)] px-4 shrink-0 overflow-hidden ${collapsed ? "justify-center px-0" : "justify-between"}`}>
      ...
      <div className={`px-3 pt-3 pb-2 shrink-0 ${collapsed ? "flex justify-center px-2" : ""}`}>
        <button
          ...
          title="New entry"
          className={`flex items-center gap-2 px-3 py-2 rounded-md border border-dashed border-[var(--border)] text-[12px] text-neutral-500 hover:text-neutral-200 hover:border-neutral-600 transition-colors cursor-pointer w-full overflow-hidden ${
      ...
      <div className="border-t border-[var(--border)] shrink-0">
        <div className="px-2 py-2">
          <Link
            ...
            className={`flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] transition-colors w-full overflow-hidden ${
              activeFilter === "trash"
        ...
        <div className="px-2 pb-2 space-y-0.5">
          <Link
            ...
            className={`flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] transition-colors overflow-hidden ${
          ...
          {isImpersonating && (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] text-orange-400 hover:text-orange-300 bg-orange-500/10 hover:bg-orange-500/20 transition-colors cursor-pointer overflow-hidden"
              title="Stop Impersonating"
            >
              <User className="w-4 h-4 shrink-0" />
              {!collapsed && <span>Stop Impersonating</span>}
            </button>
          )}

          <button
            onClick={() => setCollapsed(v => !v)}
            className="hidden md:flex w-full items-center justify-center gap-2.5 px-3 py-2 rounded-md text-[12px] text-neutral-700 hover:text-neutral-500 transition-colors cursor-pointer"
            title={collapsed ? "Expand sidebar" : "Collapse sidebar"}
          >
            {collapsed
              ? <PanelLeftOpen className="w-4 h-4 shrink-0" />
              : <><PanelLeftClose className="w-4 h-4 shrink-0" /><span className="flex-1 text-left">Collapse</span></>
            }
          </button>
        </div>
      </div>
