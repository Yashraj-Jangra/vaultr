...
async function run() {
  ...
  // Expo adaptive icon (standard Android 66dp safe-zone calibrated)
  ...
  // Standard 1:1 Lock Shield Splash Screen Asset
  await sharp(solidSvg).resize(1024, 1024).png().toFile(path.join(rootDir, 'mobile/assets/splash.png'));
  ...
  for (const m of mipmaps) {
    ...
    console.log(`Generating launcher icons for ${m.dir}...`);
  ...
  // Android native splash screen logo sizes across density buckets
  ...
  for (const s of splashDrawables) {
    const targetDir = path.join(rootDir, 'mobile/android/app/src/main/res', s.dir);
    if (!fs.existsSync(targetDir)) {
      fs.mkdirSync(targetDir, { recursive: true });
    }

    console.log(`Generating splash screen logo for ${s.dir}...`);
    // splashscreen_logo.png (standard 1:1 centered lock brand logo)
    await sharp(adaptiveSvg).resize(s.size, s.size).png().toFile(path.join(targetDir, 'splashscreen_logo.png'));
  }

  console.log('Done generating all standard-compliant app icons and splash assets!');
}
