...
// 1. Generate Adaptive Foreground SVG (transparent background with scaled, centered logo)
function getAdaptiveSvg(scale = 0.58) {
...
// 2. Generate Solid App Icon SVG (with #09090b background and proportional padding)
function getSolidSvg(scale = 0.62) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000" fill="none" shape-rendering="geometricPrecision">
...
// 3. Generate Round App Icon SVG (with circular mask for ic_launcher_round)
function getRoundSvg(scale = 0.58) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000" fill="none" shape-rendering="geometricPrecision">
...
async function run() {
  ...
  const adaptiveSvg = Buffer.from(getAdaptiveSvg(0.64));
  const solidSvg = Buffer.from(getSolidSvg(0.68));
  const roundSvg = Buffer.from(getRoundSvg(0.64));

  console.log('Generating Expo assets...');
  // Expo assets
  await sharp(adaptiveSvg).resize(1024, 1024).png().toFile(path.join(rootDir, 'mobile/assets/vaultr-lock-dark-transparent.png'));
  ...
  for (const m of mipmaps) {
    ...
    // ic_launcher_foreground.webp (transparent adaptive foreground)
