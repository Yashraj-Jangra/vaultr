...
- [x] **Monochrome Minimalist Visual Refresh (`colors.ts`, `UnlockScreen.tsx`, secondary header screens)**:
  - **White Accent Color System**: Updated global app `colors.accent` in `colors.ts` from purple (`#a78bfa`) to pure white (`#ffffff` / `rgba(255, 255, 255, 0.08)`).
  - **Div-less Plain Fingerprint Icon**: Stripped outer background box/border around biometric button on `UnlockScreen.tsx`, rendering a simple white `<Fingerprint size={28} color="#ffffff" />` icon touch target.
  - **Plain Back / Close Icon Buttons**: Removed rectangular container boxes, background colors, and borders around back/close buttons across all secondary screen headers (`AccountSettingsScreen`, `SecuritySettingsScreen`, `ItemFormScreen`, `ItemDetailScreen`, `VaultFilteredScreen`, `TrashScreen`, `FolderManagerScreen`, `DataScreen`, `SessionsScreen`).
