...
- [x] **Backend Ghost Session Fix (`auth.ts`, `verifyUser.ts`)**: Disabled Better Auth's `cookieCache` and patched `verifyUserToken` to explicitly reject sessions that have been deleted from the database. This prevents the server from blindly trusting cached session tokens for 5 minutes after a device has been remotely revoked.
...
- [x] **1:1 Exact Web Credit Card & Preview Card Parity (`ItemPreviewCard.tsx`, `ItemFormScreen.tsx`, `ItemDetailScreen.tsx`, `page.tsx`)**:
  - **Brand Network Picker**: Added Card Network selector (`Auto-detect`, `Visa`, `Mastercard`, `AMEX`, `Discover`, `RuPay`, `Other`) to mobile card creation form (`ItemFormScreen.tsx`).
